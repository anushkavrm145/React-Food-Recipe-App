...
export default function GlobalState({ children }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (data?.data?.recipes){console.log(data?.data?.recipes);setRecipeList(data.data.recipes);}
      setLoading(false);
      setSearchParam("");
      navigate('/')
    } catch (e) {
  ...
  function handleAddToFavorite(getCurrentItem){
    ...
    if(index === -1) cpyFavoriteList.push(getCurrentItem)
    else cpyFavoriteList.splice(index)
